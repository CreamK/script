...
!(async () => {
    ...
    for(let i=0;i<gladosCookie.length;i++){
        cookie=gladosCookie[i];
        
        await signin(cookie);
        await get_flows(cookie);
        await status(cookie);
        
        $.singleMsg=$.baseMsg+$.flowsMsg+$.checkinMsg;
        $.allMsg+=$.singleMsg
        $.allMsg+="\n"
    }
...
function getCookie() {
    if (
      ...
      $.log(cookie);
      $.setData(cookie, "gladosCookie");
      $.msg('🎉 🎉 🎉glados获取cookie成功!', '', cookie, {'update-pasteboard': cookie,openUrl: "Telegram://"});

    }
}

// 答应标识
async function Jsname() {
        $.log(`      [....   [..              [.....        [....      [.. .. `) 
        $.log(`    [.    [.. [..              [..   [..   [..    [.. [..    [..`)
        $.log(`   [..        [..       [..    [..    [..[..        [..[..      `)
        $.log(`   [..        [..     [..  [.. [..    [..[..        [..  [..    `)
    ...
    return new Promise((resolve) => {
      ...
      $.post(signinRequest, (error, response, data) => {
        ...
        if (JSON.stringify(time) == JSON.stringify(sysdate)) {
          change = parseInt(obj.list[0].change);
          balance = parseInt(obj.list[0].balance);
          $.checkinMsg=`今日签到获得${change}积分,账号内共有${balance}积分\n`;
        } else {
          $.checkinMsg = `签到失败\n`;
        }
    ...
    return new Promise((resolve) => {
      ...
      $.get(statusRequest, (error, response, data) => {
        ...
        if (obj.code == 0) {
          ...
          $.baseMsg = `账户：${account}\n`
          $.baseMsg += `已用${expday}天,剩余${remainday}天\n`;
...
//获取流量清空
async function get_flows(cookie){
    return new Promise((resolve) => {
      const usageRequest = {
      ...
      $.get(usageRequest, (error, response, data) => {
        ...
        if (obj.code == 0) {
          ...
          $.sum=parseInt($.sum/1000000000);
          $.flowsMsg=`本月已经使用${$.sum} G流量\n`
        } else {
          $.log(response);
          $.msg("GLaDOS", "", "❌请重新登陆更新Cookie");
        }
