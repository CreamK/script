...
if __name__=="__main__":
    msg=[]
    all_cookie=[]
    if 'xmbs' in os.environ:
        all_cookie = os.environ["xmbs"].split('&')
    for i in range(len(all_cookie)):
        print(all_cookie[i])
        userinfo=all_cookie[i].split("#")
        account=userinfo[0]
        password=userinfo[1]
        step=str(int(userinfo[2])+random.randint(100,999))
        msg.append(main(account,password,step))
    retmsg='\n'.join(msg)
    notify.send('小米步数',retmsg)
